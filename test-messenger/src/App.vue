...
export default {
  ...
  data() {
    return {
      ...
      activeContactPicture: document.querySelector(
        "div#chat-header img.contact-picture"
      ),
      activeContactName: document.querySelector(
        "div#chat-header span.users-nickname"
      ),
    };
  },
  ...
  methods: {
    async sendMessage() {
      if (this.text !== "\n") {
        ...
        const updatedMessages = await this.$requestAPI.request.sendMessage(
          newMessage
        );
    ...
    async changeContact(event) {
      ...
      const chosenContactPicturePath = document.querySelector(
        'img[name = "' + this.activeContactId + '"]'
      ).src;
      const chosenContactName = document.querySelector(
        'span[name = "' + this.activeContactId + '"]'
      ).innerHTML;
  ...
  <div id="messenger-region">
    ...
    <div id="chat-container">
      <div
        ...
        v-for="(el, index) in messagesStore.messages"
        :key="index"
      >
    ...
    <div id="send-box">
      <Textarea
        ...
        v-model="text"
        @keyup.enter.exact="
          sendMessage(), scrollToNewMessage(), cleanTextArea()
        "
      ...
      />
      <img
        @click="sendMessage(), scrollToNewMessage(), cleanTextArea()"
        class="send-sign"
        src="C:\test_task\test-messenger\icons\send-sign.svg"
        alt="send-sign"
      />
    </div>
  </div>
